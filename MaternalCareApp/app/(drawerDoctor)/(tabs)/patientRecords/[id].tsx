...
const PatientRecordDetail: React.FC = () => {
  ...
  const renderHistoryItem = ({ item }: { item: AssessmentRecord }) => {
    ...
    return (
      <TouchableOpacity 
        style={[styles.historyRow, { flexDirection: "column", paddingVertical: 12 }]}
      ...
      >
        <View style={{ flexDirection: "row" }}>
          ...
          <View style={[styles.historyCell, styles.colVitalsHistory]}>
            ...
            <Text style={styles.cellText}>Blood Sugar {item.bloodSugar} mmol/L</Text>
          ...
          <View style={{ flex: 1, justifyContent: "flex-end", backgroundColor: "rgba(0,0,0,0.5)" }}>
            <View style={{ 
              ...
              <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                <Text style={{ fontSize: 18, fontWeight: "700", color: colorScheme === "dark" ? "#F9FAFB" : "#111827" }}>
                  Assessment Details
                </Text>
              ...
              {selectedAssessment && (
                <ScrollView showsVerticalScrollIndicator={false}>
                  <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 20 }}>
                    <View>
                      <Text style={{ fontSize: 12, color: colorScheme === "dark" ? "#9CA3AF" : "#6B7280" }}>Date</Text>
                      <Text style={{ fontSize: 14, fontWeight: "600", color: colorScheme === "dark" ? "#F9FAFB" : "#111827" }}>
                        {formatDate(selectedAssessment.date)} at {formatTime(selectedAssessment.date)}
                      </Text>
                    </View>
                    <View style={{ alignItems: "flex-end" }}>
                      <Text style={{ fontSize: 12, color: colorScheme === "dark" ? "#9CA3AF" : "#6B7280" }}>Risk Level</Text>
                      <View style={[
                        styles.riskPill,
                        selectedAssessment.riskLevel.toLowerCase().includes("high") ? styles.riskHigh :
                        (selectedAssessment.riskLevel.toLowerCase().includes("mid") || selectedAssessment.riskLevel.toLowerCase().includes("medium")) ? styles.riskModerate :
                        styles.riskLow,
                        { marginTop: 4 }
                      ]}>
                        <Text style={styles.riskText}>{selectedAssessment.riskLevel}</Text>
                      </View>
                    </View>
                  </View>

                  <Text style={{ fontSize: 16, fontWeight: "600", color: colorScheme === "dark" ? "#F9FAFB" : "#111827", marginBottom: 10 }}>
                    Vital Signs
                  </Text>
                  <View style={{ backgroundColor: colorScheme === "dark" ? "#374151" : "#F3F4F6", borderRadius: 12, padding: 16, marginBottom: 20 }}>
                    <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 8 }}>
                      <Text style={{ color: colorScheme === "dark" ? "#D1D5DB" : "#4B5563" }}>Blood Pressure</Text>
                      <Text style={{ fontWeight: "600", color: colorScheme === "dark" ? "#F9FAFB" : "#111827" }}>{selectedAssessment.systolic}/{selectedAssessment.diastolic} mmHg</Text>
                    </View>
                    <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 8 }}>
                      <Text style={{ color: colorScheme === "dark" ? "#D1D5DB" : "#4B5563" }}>Blood Sugar</Text>
                      <Text style={{ fontWeight: "600", color: colorScheme === "dark" ? "#F9FAFB" : "#111827" }}>{selectedAssessment.bloodSugar} mmol/L</Text>
                    </View>
                    <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 8 }}>
                      <Text style={{ color: colorScheme === "dark" ? "#D1D5DB" : "#4B5563" }}>Heart Rate</Text>
                      <Text style={{ fontWeight: "600", color: colorScheme === "dark" ? "#F9FAFB" : "#111827" }}>{selectedAssessment.heartRate} bpm</Text>
                    </View>
                    <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                      <Text style={{ color: colorScheme === "dark" ? "#D1D5DB" : "#4B5563" }}>Body Temp</Text>
                      <Text style={{ fontWeight: "600", color: colorScheme === "dark" ? "#F9FAFB" : "#111827" }}>{selectedAssessment.bodyTemp.toFixed(1)} °C</Text>
                    </View>
                  </View>

                  {selectedAssessment.possible_maternal_risks && selectedAssessment.possible_maternal_risks.length > 0 && (
                    <View style={{ marginBottom: 20 }}>
                      <View style={{ flexDirection: "row", alignItems: "center", marginBottom: 10 }}>
                        <AlertTriangle size={18} color="#E11D48" style={{ marginRight: 8 }} />
                        <Text style={{ fontSize: 16, fontWeight: "600", color: colorScheme === "dark" ? "#F9FAFB" : "#111827" }}>
                          Possible Risks
                        </Text>
                      </View>
                      {selectedAssessment.possible_maternal_risks.map((risk, idx) => (
                        <View key={idx} style={{ flexDirection: "row", marginBottom: 6 }}>
                          <View style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: "#E11D48", marginTop: 6, marginRight: 8 }} />
                          <Text style={{ flex: 1, color: colorScheme === "dark" ? "#D1D5DB" : "#4B5563" }}>{risk}</Text>
                        </View>
                      ))}
                    </View>
                  )}
                    ...
                    <View style={{ marginBottom: 20 }}>
                      ...
                      {selectedAssessment.recommendations.map((rec, idx) => (
                        <View key={idx} style={{ flexDirection: "row", marginBottom: 8 }}>
                          ...
                          <Text style={{ flex: 1, color: colorScheme === "dark" ? "#D1D5DB" : "#4B5563" }}>{rec}</Text>
                        </View>
                      ))}
                    </View>
                  )}
                  ...
                  {selectedAssessment.note ? (
                    <View style={{ marginBottom: 40 }}>
                      ...
                      <Text style={{ color: colorScheme === "dark" ? "#D1D5DB" : "#4B5563", fontStyle: "italic" }}>
                        {selectedAssessment.note}
                      </Text>
                    </View>
